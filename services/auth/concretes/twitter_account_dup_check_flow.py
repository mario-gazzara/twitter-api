...
class TwitterAccountDuplicationCheckFlow(AbstractFlow):
    def handle(self, context: TwitterAuthContext) -> str:
        if context.subtask_id == TwitterAuthFlows.ACCOUNT_DUPLICATION_CHECK.value:
            payload = {
            ...
            if not response.is_success or response.data is None:
                raise Exception('Failed to execute account duplication check subtask')
